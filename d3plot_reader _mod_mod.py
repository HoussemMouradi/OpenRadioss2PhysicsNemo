...
def parse_k_file(k_file_path: str) -> dict[int, float]:
    """
    I'll try to parse radioss starter to get part ID and there correspondante thickness values, 
    ...
    while i < len(lines):
        ...
        if line.startswith("/PART/"):
            ...
            section_id = int(line[12:15])
            tokens = lines[i + 4].split()
            section_thickness[section_id] = tokens[2]
            i += 1
