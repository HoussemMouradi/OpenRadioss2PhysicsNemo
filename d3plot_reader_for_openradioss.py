...
def find_run_folders(base_data_dir: str) -> list[str]:
    ...
    if os.path.isdir(base_data_dir):
        for item in os.listdir(base_data_dir):
            ...
            for other_item in os.listdir(item_path):
                if os.path.isdir(item_path) and other_item.endswith(".d3plot"):
                    run_dirs.append(item)
    return run_dirs


def parse_starter_file(starter_file_path: str) -> dict[int, float]:
    """
    Args:
        Radioss starter "_0000.rad" file.

    Returns:
        Dictionary mapping part ID -> thickness.
    """
    part_to_prop: dict[int, int] = {}
    prop_thickness: dict[int, float] = {}
    ...
    while i < len(lines):
        ...
        if line.startswith("/PART/"):
            part_id = int(line[6:10])
            i += 1
            if i + 1 < len(lines):
                tokens = lines[i + 1].split()
                if len(tokens) >= 2:
                    try:
                        prop_id = int(tokens[1])
                        #section_id is equivalent to /prop/ in radioss starter
                        part_to_prop[part_id] = prop_id
                    except ValueError:
                        pass
        elif line.startswith("/PROP/SHELL/"):
            prop_id = int(line[12:15])
            tokens = lines[i + 4].split()
            prop_thickness[prop_id] = tokens[2]
            i += 1

        else:
            i += 1

    return { pid: prop_thickness.get(sid, 0.0) for pid, sid in part_to_prop.items() }
...
def process_d3plot_data(
    ...
    for run_i, run_folder in enumerate(sorted(run_folders)):
        ...
        data_path = os.path.join(data_dir, run_folder, "Bumper_Beam_AP_meshed.d3plot")
        ...
        for f in os.listdir(os.path.join(data_dir, run_folder)):
            if f.endswith("0000.rad"):
                starter_file_path = os.path.join(data_dir, run_folder, f)
                break

        if starter_file_path and os.path.exists(starter_file_path):
            part_thickness_map = parse_starter_file(starter_file_path)
            node_thickness = compute_node_thickness(
                mesh_connectivity, part_ids, part_thickness_map, actual_part_ids
            )
        else:
            logger.warning(f"No starter file in {run_folder}, defaulting thickness=0")
